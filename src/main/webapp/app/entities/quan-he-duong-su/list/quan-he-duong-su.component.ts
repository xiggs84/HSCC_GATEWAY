...
export class QuanHeDuongSuComponent implements OnInit {
  ...
  trackIdQuanHe = (_index: number, item: IQuanHeDuongSu): number => this.quanHeDuongSuService.getQuanHeDuongSuIdentifier(item);
  ...
  ngOnInit(): void {
    this.subscription = combineLatest([this.activatedRoute.queryParamMap, this.activatedRoute.data])
      .pipe(
        ...
        tap(() => {
          if (!this.quanHeDuongSus || this.quanHeDuongSus.length === 0) {
            this.load();
          }
        }),
      )
